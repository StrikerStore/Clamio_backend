...
const CURRENT_DEPLOY_VERSION = '2026-01-25-tracking-api-migration'; // Change this for each deploy that needs the sync
...
async function runDeployTasks() {
    ...
    // ============================================
    // TASK 1: Sync All Tracking Status
    // ============================================
    const TASK_SYNC_TRACKING = 'sync-all-tracking-status';

    if (hasTaskRun(state, TASK_SYNC_TRACKING)) {
        console.log(`✅ [SKIP] Task "${TASK_SYNC_TRACKING}" already completed for this deploy`);
    } else {
        console.log(`\n🔄 [RUN] Task: ${TASK_SYNC_TRACKING}`);
        console.log('-'.repeat(60));

        try {
            // Run the sync script
            execSync('node scripts/sync-all-tracking-status.js', {
            ...
            state.tasksCompleted.push(TASK_SYNC_TRACKING);
            saveDeployState(state);
            console.log(`✅ Task "${TASK_SYNC_TRACKING}" completed successfully`);
        } catch (error) {
            console.error(`❌ Task "${TASK_SYNC_TRACKING}" failed:`, error.message);
